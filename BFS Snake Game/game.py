...
def game_loop(settings):
    ...
    pathfinder = BFSPathfinder(settings['width'], settings['height'], GRID_SIZE)
    ...
    while running:
        ...
        if not game_over:
            ...
            for _ in range(moves):
                # Dynamic path recalculation with BFS
                ...
                # Validate and use path
                if not current_path or len(current_path) < 2:
                    game_over = True
                    break
                
                # Get next direction from BFS path
